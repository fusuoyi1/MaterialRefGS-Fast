...
for ((i=1; i<=9; i++)); do
    ...
    if [ -n "$stat2" ] && [[ "$stat2" =~ ^[0-9]+$ ]] && [ "$stat2" -lt 2000 ]; then
...
if [ "$gpuid" -ge 0 ]; then
    ...
    # Scene configuration
    scene=teapot  # Change this to your scene name
    data_dir=/home/shiqingaoxue/文档/3DGS/MaterialRefGS/datasets/GlossySynthetic
    ...
    # Output directory
    output_dir=/home/shiqingaoxue/文档/3DGS/MaterialRefGS/output_glossy
