...
struct Triangle {
    ...
    static bool compareVector3f(const Eigen::Vector3f& v1, const Eigen::Vector3f& v2) {  
        if (v1.x() != v2.x()) {  
            return v1.x() < v2.x();   
        } else if (v1.y() != v2.y()) {  
            return v1.y() < v2.y();  
    ...
    __host__ bool operator <(const Triangle& other) const {
        if (compareVector3f(a, other.a)) {
            return true;  
        } else if (compareVector3f(other.a, a)) {  
            return false;  
        } else if (compareVector3f(b, other.b)) {  
            return true;  
        } else if (compareVector3f(other.b, b)) {  
            return false;  
        } else {  
            return compareVector3f(c, other.c);  
        }  
    }
