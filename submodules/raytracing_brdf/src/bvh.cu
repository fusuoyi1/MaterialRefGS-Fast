...
public:
    ...
    void ray_trace_gpu(uint32_t n_elements, const float* rays_o, const float* rays_d, float* positions, float* normals, float* depth, int* triangle_indices ,const Triangle* gpu_triangles, cudaStream_t stream) override {
...
__global__ void raytrace_kernel(uint32_t n_elements, const Vector3f* __restrict__ rays_o, const Vector3f* __restrict__ rays_d, Vector3f* __restrict__ positions, Vector3f* __restrict__ normals, float* __restrict__ depth, int* __restrict__  triangle_indices, const TriangleBvhNode* __restrict__ nodes, const Triangle* __restrict__ triangles) {
    ...
    if (p.first >= 0) {
        ...
        // normals[i] = triangles[p.first].a;
        // normals[i] = {p.first,p.first,p.first};
    } else {
        normals[i].setZero();
    }
