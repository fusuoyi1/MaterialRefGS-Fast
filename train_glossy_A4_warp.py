...
def load_normal_prior(dataset, scan_id:str, r=1, method="GeoWizard", args=None):
    ...
    def get_mask_dir(dataset_path):
        ...
        if "GlossySynthetic" in dataset_path:
    ...
    return mask_images, normal_images
...
def training(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, model_path, args, debug_from=None):
    ...
    #新加的
    # =========================
    # Compare logging helpers
    # =========================
    EXPERIMENT_TAG = "A4_warp"
    PRINT_COMPARE_INTERVAL = 1000
    VISUAL_INTERVAL = 2000

    WARP_START = 25000
    WARP_INTERVAL_STAGE1 = 4  # 25000 ~ 30000
    WARP_INTERVAL_STAGE2 = 8  # 30000+
    ...
    densify_call_count = 0
    prune_only_call_count = 0
    ...
    print(f"PRINT_COMPARE_INTERVAL: {PRINT_COMPARE_INTERVAL}")
    print(f"TEST_INTERVAL: {TEST_INTERVAL}")
    print(f"VISUAL_INTERVAL: {VISUAL_INTERVAL}")
    print(f"MESH_EXTRACT_INTERVAL: {MESH_EXTRACT_INTERVAL}")
    print(f"WARP_START: {WARP_START}")
    print(f"WARP_INTERVAL_STAGE1: {WARP_INTERVAL_STAGE1}")
    print(f"WARP_INTERVAL_STAGE2: {WARP_INTERVAL_STAGE2}")
    print("PSNR display source: cached formal evaluation")
    print("Densify schedule: adaptive interval + adaptive grad threshold")
    #print("A3_1 note: adaptive densify starts right after init stage")
    print("A4_warp note: stronger warp downsampling (4 / 8)")
    print("=======================================")
    # 新加的
    ...
    mask_images, normal_images = load_normal_prior(dataset, scene_name, dataset.resolution, args=args)
    normal_gamma = 2
    ...
    while iteration < TOT_ITER:
        ...
        # 新加的
        if iteration <= 30000:
            cur_warp_interval = WARP_INTERVAL_STAGE1
        else:
            cur_warp_interval = WARP_INTERVAL_STAGE2
        do_warp = (iteration > WARP_START) and (iteration % cur_warp_interval == 0)
        if do_warp:
            warp_call_count += 1
            geo_loss, ncc_loss, ncc_loss_basecolor, metallic_warp_loss, roughness_warp_loss, d_mask, ref_weight, base_color_loss_map = calc_warp_loss(
        ...
        with torch.no_grad():
            ...
            time_acc_ms += iter_time_ms
            time_acc_count += 1
            # 新加的

            if iteration % VISUAL_INTERVAL == 0 or iteration == first_iter + 1 or iteration == opt.volume_render_until_iter + 1:
                save_training_vis(
                    viewpoint_cam, gaussians, background, render, pipe, opt,
                    iteration, initial_stage, d_mask, ref_weight, base_color_loss_map, render_pkg, None
                )
            ...
            # A2.1: only refresh cached PSNR on formal evaluation iterations
            if iteration in testing_iterations:
                psnr_test_eval = evaluate_psnr(
                    scene, render, {"pipe": pipe, "bg_color": background, "opt": opt}
                )
            if iteration % 10 == 0:
                loss_dict = {
                    ...
                    "PSNR-test": f"{psnr_test_eval:.{4}f}"
                }
                progress_bar.set_postfix(loss_dict)
                progress_bar.update(10)
                # 新加的
                if iteration % PRINT_COMPARE_INTERVAL == 0:
                    cur_points = len(gaussians.get_xyz)
                    #delta_points = cur_points - last_points if last_points != 0 else 0
                    delta_points = cur_points if last_points == 0 else (cur_points - last_points)
                    avg_iter_ms = time_acc_ms / max(time_acc_count, 1)

                    print(
                        f"\n[COMPARE][{EXPERIMENT_TAG}][ITER {iteration}] "
                        f"avg_iter_ms={avg_iter_ms:.2f} | "
                        f"points={cur_points} | delta_points={delta_points:+d} | "
                        f"psnr_train={ema_psnr_for_log:.4f} | psnr_test={psnr_test_eval:.4f} | "
                        f"loss={ema_loss_for_log:.5f} | "
                        f"geo={ema_loss_geo_loss:.5f} | pho={ema_loss_pho_loss:.5f} | "
                        f"bc={ema_loss_base_color:.5f} | mw={ema_loss_metallic_warp:.5f} | rw={ema_loss_roughness_warp:.5f} | "
                        f"warp_calls={warp_call_count} | warp_skips={warp_skip_count} | "
                        f"densify_calls={densify_call_count} | prune_only_calls={prune_only_call_count} | "
                        f"densify_progress={densify_progress:.2f} | "
                        f"densify_interval={cur_densification_interval} | "
                        f"densify_grad={cur_densify_grad_threshold:.6f}"
                    )
            ...
            if iteration < opt.densify_until_iter and iteration != opt.volume_render_until_iter:
                # 新加的
                did_densify_this_iter = False
                densify_progress = (iteration - opt.densify_from_iter) / max(
                    1, (opt.densify_until_iter - opt.densify_from_iter)
                )
                densify_progress = max(0.0, min(1.0, densify_progress))
                #新加的
                ...
                if iteration <= opt.init_until_iter:
                    ...
                    cur_densification_interval = 100
                    cur_densify_grad_threshold = opt.densify_grad_threshold
                else:
                    opacity_reset_intval = 3000

                    if densify_progress < 0.33:
                        cur_densification_interval = 100
                        cur_densify_grad_threshold = opt.densify_grad_threshold
                    elif densify_progress < 0.66:
                        cur_densification_interval = 200
                        cur_densify_grad_threshold = 1.5 * opt.densify_grad_threshold
                    else:
                        cur_densification_interval = 300
                        cur_densify_grad_threshold = 2.0 * opt.densify_grad_threshold
                ...
                #                                size_threshold)
                # 新加的
                if iteration > opt.densify_from_iter and iteration % cur_densification_interval == 0:
                    did_densify_this_iter = True
                    densify_call_count += 1
                    size_threshold = 20 if iteration > opt.opacity_reset_interval else None
                    gaussians.densify_and_prune(
                        cur_densify_grad_threshold,
            ...
            if (
                    (iteration >= opt.indirect_from_iter and iteration % MESH_EXTRACT_INTERVAL == 0)
                    or iteration == opt.indirect_from_iter
                ...
                if not HAS_RESET0:
                    ...
                    print(f"[ITER {iteration}] Mesh exported to {ply_path}")
                    gaussians.update_mesh(mesh)
