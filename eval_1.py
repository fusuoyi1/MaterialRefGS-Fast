...
def render_set(model_path, views, gaussians, pipeline, background, save_ims, opt,render):
    if save_ims:
        ...
        dir_names = ["rgb","gt","normal","surf_normal","surf_depth","diffuse_map","specular_map","albeldo_map","roughness_map","refl_strength_map","direct_light","visibility","indirect_light"]#"direct_light","visibility","indirect_light"]
    ...
    for idx, view in enumerate(tqdm(views, desc="Rendering progress")):
        ...
        #gt = gt[None, 0:3, :, :]
        gt = gt[None, 0:3, :, :].to(render_color.device, dtype=render_color.dtype)
...
def render_sets(dataset: ModelParams, iteration: int, pipeline: PipelineParams, save_ims: bool, op, indirect):
    with torch.no_grad():
        ...
        if eval_iteration < 0:
            eval_iteration = searchForMaxIteration(os.path.join(dataset.model_path, "point_cloud"))
